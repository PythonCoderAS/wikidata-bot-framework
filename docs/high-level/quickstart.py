...
class MyBot(PropertyAdderBot):
    ...
    def run_item(self, item):
        ...
        extra_qual = ExtraQualifier.from_property_id_and_value(
            "P580", "2020-01-01T00:00:00Z"
        )  # qualifier: start time: 2020-01-01
        extra_ref = ExtraReference()
        claim = Claim(site, "P854")
        claim.setTarget("https://www.example.com")
        extra_ref.add_claim(
            claim, True
        )  # reference: stated in: https://www.example.com
...
def run_bot():
    bot = MyBot()
    bot.act_on_item(ItemPage(site, "Q4115189"))
